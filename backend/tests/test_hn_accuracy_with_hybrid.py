"""
Hacker News Sentiment Accuracy Test - Using ACTUAL Hybrid VADER Model

This test uses your real production Hybrid VADER model 
(Enhanced VADER + Logistic Regression ensemble) instead of basic VADER.
"""
...
# Add backend to path for direct model imports (avoiding service initialization)
backend_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, backend_path)

from datetime import datetime

# Direct imports to avoid triggering service initialization that needs asyncpraw
# We import the model modules directly
import importlib.util

# Load hybrid_vader_model directly
hybrid_spec = importlib.util.spec_from_file_location(
    "hybrid_vader_model", 
    os.path.join(backend_path, "app", "service", "sentiment_processing", "models", "hybrid_vader_model.py")
)
hybrid_module = importlib.util.module_from_spec(hybrid_spec)

# First load the base sentiment_model
base_spec = importlib.util.spec_from_file_location(
    "sentiment_model",
    os.path.join(backend_path, "app", "service", "sentiment_processing", "models", "sentiment_model.py")
)
base_module = importlib.util.module_from_spec(base_spec)
sys.modules['sentiment_model'] = base_module
base_spec.loader.exec_module(base_module)

# Now we can load hybrid vader
sys.modules['hybrid_vader_model'] = hybrid_module
hybrid_spec.loader.exec_module(hybrid_module)

HybridVADERModel = hybrid_module.HybridVADERModel
HybridConfig = hybrid_module.HybridConfig

# Load finbert_model directly
finbert_spec = importlib.util.spec_from_file_location(
    "finbert_model",
    os.path.join(backend_path, "app", "service", "sentiment_processing", "models", "finbert_model.py")
)
finbert_module = importlib.util.module_from_spec(finbert_spec)
sys.modules['finbert_model'] = finbert_module
finbert_spec.loader.exec_module(finbert_module)

FinBERTModel = finbert_module.FinBERTModel
